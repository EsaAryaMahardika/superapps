<div class="pengurus-item flex items-center justify-between p-3 rounded-xl bg-[#F4F7FE] hover:bg-[#EEF2FF] transition-colors"
    data-search="{{ strtolower($p->nama . ' ' . $p->nis . ' ' . ($jabNama ?? '') . ' ' . ($divNama ?? '') . ' pengurus ' . ($p->asal ?? 'dalam')) }}">
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <div class="flex items-center gap-1.5">
                ...
                <span class="px-1.5 py-0.5 rounded-md text-[10px] font-semibold {{ ($p->asal ?? 'dalam') === 'luar' ? 'bg-amber-100 text-amber-700' : 'bg-gray-200/70 text-gray-600' }}">
    ...
    <div class="flex items-center gap-2">
        <button
            onclick="openEditPengurus('{{ $p->nis }}','{{ addslashes($p->nama) }}',{{ $p->jabatan_id ?? 'null' }},'{{ $p->asal ?? 'dalam' }}')"
